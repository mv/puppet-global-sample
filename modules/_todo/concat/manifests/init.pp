...
define concat($mode = 0644,
    ...
    case $warn {
        true,yes,on:  { $warnmsg = $default_warn_message }
        false,no,off: { $warnmsg = '' }
...
#   $warnmsg_escaped = regsubst($warnmsg, "'", "'\\\\''", 'G')
    $warnmsg_escaped = regsubst($warnmsg, "'${lint_null}", "'\\\\''${lint_null}", 'G')
    ...
    case $force {
        true,yes,on: { $forceflag = '-f' }
        false,no,off: { $forceflag = '' }
        default: { fail("Improper 'force' value given to concat: ${force}") }
    }

    case $gnu {
        true,yes,on: { $gnuflag = '' }
        false,no,off: { $gnuflag = '-g' }
